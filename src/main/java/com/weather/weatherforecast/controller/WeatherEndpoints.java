...
public class WeatherEndpoints {
	...
	public ResponseEntity<WeatherForecastResponse> getWeatherForecast(@PathVariable("city") String city) {
		WeatherForecastResponse weatherForecastResponse = new WeatherForecastResponse();
		try {
			if(!city.isBlank()) {
				weatherForecastResponse = weatherForecastService.weatherAPIForecast(city);
			}
			else {
		...
		return new ResponseEntity<WeatherForecastResponse>(weatherForecastResponse, HttpStatus.OK);
	}
}
